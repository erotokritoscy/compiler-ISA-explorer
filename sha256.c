// sha256_single.c - SHA-256 implementation with demo
// Compile: gcc -O2 -std=c11 sha256_single.c -o sha256
// Run:     ./sha256
...
int main(void) {
    m5_reset_stats(0, 0);
    const char *msg = "llvm compiler";
    char hex[65];
    sha256_easy_hash_hex(msg, strlen(msg), hex);
    m5_dump_stats(0, 0);
    printf("SHA-256(\"%s\") = %s\n", msg, hex);
