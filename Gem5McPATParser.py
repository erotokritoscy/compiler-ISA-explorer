...
def prepareTemplate(outputFile):
    config["system"]["cpu"] = [config["system"]["cpu"]]
...
def getConfValue(confStr):
    ...
    for x in spltConf:
        ...
        # print("x:")
        # print(x)
        if x.isdigit() and isinstance(currConf, list):
            currConf = currConf[int(x)]
        elif isinstance(currConf, dict) and x in currConf:
